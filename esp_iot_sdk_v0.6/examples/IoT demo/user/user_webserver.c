...
version_get(struct jsontree_context *js_ctx)
{
    ...
    if (os_strncmp(path, "major", SDK_VERSION_MAJOR) == 0) {
        jsontree_write_int(js_ctx, 1);
    } else if (os_strncmp(path, "minor", SDK_VERSION_MINOR) == 0) {
...
JSONTREE_OBJECT(version_tree,
                JSONTREE_PAIR("major", &version_callback),
                JSONTREE_PAIR("minor", &version_callback));
...
{
    rst_parm *prst_parm = arg;

    if (prst_parm != NULL) {
        switch (prst_parm->parmtype) {
            case WIFI:
                if (prst_parm->pespconn->state == ESPCONN_CLOSE) {
                    ...
                    if (os_strcmp(pURL_Frame->pFilename, "wifi") == 0) {
                        if (pParseBuffer != NULL) {
                            ...
                            if (sta_conf->ssid[0] != 0x00 || ap_conf->ssid[0] != 0x00) {
                                ...
                                os_timer_setfn(restart_10ms, (os_timer_func_t *)restart_10ms_cb, rstparm);
                        ...
                        if (pParseBuffer != NULL) {
                            struct jsontree_context js;
                            //os_printf("wjl test rcv command light \n");
...
user_webserver_init(uint32 port)
{
    ...
    esp_conn.proto.tcp->server_port = port;
